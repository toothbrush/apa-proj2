...
\begin{table}
    \centering
    \begin{tabular}{rcp{15cm}}
        \hline
        $\mathcal{U}_{CFA}(Nat,Nat)$ & = & $id$ \\
        $\mathcal{U}_{CFA}(Bool,Bool)$ & = & $id$ \\
        $\mathcal{U}_{CFA}(\tau_1,\tau_2)$ & = & \parbox{\textwidth}{$ \left\{
\begin{array}{ll}
    id                      & if \tau_1 = \tau_2         \\
...
%                                        theta2 = unifyAnn (applySubst theta1 an) 
%                                                          (applySubst theta1 an')
...
% 
        \hline
    \end{tabular}
    \caption{Unification algorithm for lambda calculus including data structures.}
    \label{tab:algoU}
\end{table}
\section{Approach}
